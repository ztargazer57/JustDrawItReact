...
function TextContent({ content = "", author = "" }: TextContentProps) {
  if (author == "") {
    ...
    return (
      <div className="text-jdbrown text-4xl h-100 w-full p-10 flex flex-col">
        <p className="text-center">{content}</p>
        <div className="mt-10 self-end text-2xl">
          <p>- {author}</p>
        </div>
      </div>
    );
...
function HomeLabel({ label, color }: HomeLabelProps) {
  return (
    <div
      className="py-2 px-4  text-2xl h-fit w-fit text-jdbrown"
